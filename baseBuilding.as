package
{
   ...
   public class baseBuilding extends Sprite
   {
      ...
      private function work(event:Event) : void
      {
         var usePos:* = 0;
         var copyArray:* = null;
         var scan_b:* = undefined;
         var missileTarget:* = null;
         var vars:* = null;
         var q:* = undefined;
         var wi:int = 0;
         var ranTot:* = null;
         var ranMan:Number = NaN;
         var d:* = undefined;
         var ovrR:Boolean = false;
         var canCancelGo2:Boolean = false;
         var ir4:* = undefined;
         var canCancelGo:Boolean = false;
         var i:* = undefined;
         ...
         else
         {
            if(gameKon.theRoot.gameRunning && (gameKon.theRoot.singlePlayerMode || ownBy == gameKon.playerNow || ownBy.aiOn && gameKon.theRoot.roomMaster))
            {
               if(aiCheck != null)
               {
                  ...
                  usePos = uint(ownBy.POS);
               }
               else
               {
                  usePos = uint(gameKon.theRoot.clientPlayer[1]);
               }
            ...
            else if(!gameKon.theRoot.singlePlayerMode)
            {
