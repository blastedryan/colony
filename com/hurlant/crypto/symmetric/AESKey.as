package com.hurlant.crypto.symmetric
{
   ...
   public class AESKey implements ISymmetricKey
   {
      ...
      private function expandKey() : void
      {
         ...
         Nr = Nk + 6;
         for(idx = uint(Nk); idx < Nb * (Nr + 1); idx++)
         {
            tmp0 = uint(key[4 * idx - 4]);
            tmp1 = uint(key[4 * idx - 3]);
            tmp2 = uint(key[4 * idx - 2]);
            tmp3 = uint(key[4 * idx - 1]);
            if(!(idx % Nk))
            {
               tmp4 = uint(tmp3);
               tmp3 = uint(Sbox[tmp0]);
