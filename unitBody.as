package
{
   ...
   public class unitBody extends displayBody
   {
      ...
      public function unitBody(id:uint, p:playerKon, rand:Number = -1)
      {
         life = maxLife;
      ...
      private function go(event:Event) : void
      {
         if(aiPosition && !kiteNow && !aiOverride)
         {
            ...
            else if(x > aiPoint + 25 && normalDir == 1 || x < aiPoint - 25 && normalDir == -1)
            {
               makeOption(3);
            }
            else
      ...
      private function getRange(m:unitBody, n:weapon, getC:Number = 0, retVal:Boolean = false) : *
      {
         ...
         if(m.ID == 0 || ID == 0)
         {
            theDist = Number(theDist - 100);
         }
         if(m.airUnit || airUnit)
         {
            theDist = Number(theDist - 50);
         }
         if(retVal)
         {
