package
{
   ...
   public class MultiplayerScreen extends MovieClip
   {
      ...
      public function MultiplayerScreen(userName:String, team:uint, color:uint, userData:Object, s:Stage, n:String, usr:uint)
      {
         var hg:* = undefined;
         eSend = new Object();
         ...
         var nCyAr:Array = new Array("Lobby","Veteran Hall","Team Channel","Deutsch Zimmer","AFK Garden");
         foxRooms = ["MLX_6_Lobby","MLX_6_Veteran Hall","MLX_6_Team Channel","MLX_6_Deutsch Zimmer","MLX_6_AFK Garden"];
         for(hg in nCyAr)
         {
      ...
      private function runChatScript(caseString:uint, caseStringSub:uint, errorString:String, chatRef:*) : void
      {
         ...
         var tName:* = null;
         var rankST:* = null;
         var foundTheGuy:Boolean = false;
         var subArray:Array = new Array();
         var foxOverride:Boolean = false;
      ...
      private function keyInput(ke:KeyboardEvent) : void
      {
         ...
         if(ke.keyCode == 13)
         {
            ...
            else if(chatRef.chatSend.text.indexOf("/add") == 0)
            {
               runChatScript(2,5,"??",chatRef);
            }
            else if(chatRef.chatSend.text.indexOf("/remove") == 0)
            {
               runChatScript(3,8,"??",chatRef);
            }
            else if(chatRef.chatSend.text.indexOf("/block") == 0)
            {
               mpKon.blockBuddy(chatRef.chatSend.text.substr(7),true);
               chatRef.chatSend.text = "";
            }
            else if(chatRef.chatSend.text.indexOf("/unblock") == 0)
            {
               mpKon.blockBuddy(chatRef.chatSend.text.substr(9),false);
               chatRef.chatSend.text = "";
            }
            else if(chatRef.chatSend.text.indexOf("/w") == 0)
            {
            ...
            else if(chatRef.chatSend.text.indexOf("/friendlist") == 0)
            {
               showBuddyInfo();
               chatRef.chatSend.text = "";
            }
            else if(chatRef.chatSend.text.indexOf("/kick") == 0)
            {
               if(roomMaster)
               {
                  if(gameRoomMode)
            ...
            else if(userName.indexOf("guest_") == -1 && userName != "")
            {
               if(c_countDown == 0)
               {
                  sendMessage();
      ...
      public function updateCList_Join(a:Array, b:*) : void
      {
         ...
         var saveIndex:* = 0;
         var j:* = undefined;
         var indexBefore:* = undefined;
         var labelBefore:* = undefined;
         var i:int = 0;
         var v:* = undefined;
      ...
      public function updateCList_Leave(a:Array, b:*) : void
      {
         ...
         if(gameRoomMode && roomMaster)
         {
            for(j in cBoxArray)
            {
               ...
               for(e = 0; e < cBoxArray[j].length; e++)
               {
      ...
      public function updateDBOXinGame(a:Array) : void
      {
         ...
         for(i in dBoxArray)
         {
            ...
            for(u in foxUsers)
            {
               ...
               {
                  theRank = uint(foxUsers[u].getVariables()["rank"]);
                  if(theRank == 100)
                  {
                     theRank = 0;
                  }
      ...
      private function upTeamHand(e:Event) : void
      {
         ...
         {
            justClosed = false;
            theSel = uint(lastBox.selectedIndex);
            for(j in cBoxArray)
            {
